...
#-- Filter
$users = @(Get-ADUser -filter * -Properties SamAccountName, Name, Office,Enabled, PasswordNeverExpires, PasswordLastSet, Modified)

$result = @($users | Select-Object SamAccountName, Name, Office,Enabled, PasswordNeverExpires, PasswordLastSet, Modified)
...
$result = $result | Sort-Object -property Enabled -descending

#-- Display result on screen
#$result | ft -auto 

$result | Export-CSV -Path c:\temp\adusers.csv
